 ...
 * This is inspired by the [Inflector](http://api.rubyonrails.org/classes/Inflector.html) class in
 * [Ruby on Rails](http://www.rubyonrails.org), which is distributed under the
 * [Rails license](http://wiki.rubyonrails.org/rails/pages/License).
 *
 * @author Randall Hauch
 */
class Inflector {
    inner class Rule constructor(
        private val expression: String,
        replacement: String?
    ) {
        private val expressionPattern: Pattern =
            Pattern.compile(this.expression, Pattern.CASE_INSENSITIVE)
        ...
        override fun equals(other: Any?): Boolean {
            ...
            if (other != null && other.javaClass == this.javaClass) {
                ...
                if (expression.equals(that.expression, ignoreCase = true)) return true
            }
            return false
        }
     ...
     * [.getUncountables].
     */
    private val uncountables: MutableSet<String> = HashSet()
    ...
    fun camelCase(
       ...
       val trimmedLowerCaseAndUnderscoredWord = lowerCaseAndUnderscoredWord.trim { it <= ' ' }
        if (trimmedLowerCaseAndUnderscoredWord.isEmpty()) return ""
        if (uppercaseFirstLetter) {
            var result = trimmedLowerCaseAndUnderscoredWord
            // Replace any extra delimiters with underscores (before the underscores are converted in the next step)...
            if (delimiterChars.isNotEmpty()) {
        ...
        return ("" + lowerCaseAndUnderscoredWord[0].lowercaseChar() + camelCase(
    ...
    fun capitalize(words: String): String {
        ...
        if (result.isEmpty()) return ""
        if (result.length == 1) return result.uppercase(Locale.getDefault())
        return "" + result[0].uppercaseChar() + result.substring(1).lowercase(Locale.getDefault())
    }
    ...
    fun humanize(
        ...
        // Remove all the tokens that should be removed
    ...
    fun ordinalNumber(number: Int): String {
        ...
        if (number in 11..13) return numberStr + "th"
    ...
    fun addPluralize(
        ...
        val pluralizeRule: Rule = Rule(rule, replacement)
    ...
    fun addSingularize(
        ...
        val singularizeRule: Rule = Rule(rule, replacement)
